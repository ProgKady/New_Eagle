...
_isPython = (typeof _python=='undefined' ? false : _python );
...
function DsApp()
{
	...
	/*#app*/ this.StartDebugServer = function( options ) { prompt( "#", "App.StartDebugServer(\f"+options ); }
	...
	/*#app*/ this.HasExternalStorage = function() { return prompt( "#", "App.HasExternalStorage(" )=="true"; }
	...
	/*#app*/ this.Broadcast = function( type,msg,options ) { prompt( "#", "App.Broadcast(\f"+type+"\f"+msg+"\f"+options ); }
	...
	/*#app*/ this.HideDialogs = function() { prompt( "#", "App.HideDialogs(" ); }
	...
	/*#app*/ this.CopyFolder = function( src,dest,overwrite,filter,options ) { prompt( "#", "App.CopyFolder(\f"+src+"\f"+dest+"\f"+overwrite+"\f"+filter+"\f"+options); }
	...
	/*#app*/ this.Print = function() { prompt( "#", "App.Print(" ); }
    ...
    /*#app*/ this.Include = function( url ) {
        _LoadScriptSync( url, true )
    }

    //app.Import( "PocketBase", "ds:Html/pocketbase.es.js" )
    //app.Import( "* as utils", "./utils.js" )
    /*#app*/ this.Import = function( obj, url ) {
        if( _scripts[url] ) return
        var head = document.getElementsByTagName('head')[0]
        var script = document.createElement('script')
        var name = obj.split(" ").slice(-1)
        script.type = "module"
        script.text = "import "+obj+" from '"+url+"'; window."+name+" = "+name
	...
	//For Python.
	/*#app*/ this.SetTimeout = function( callback, delay, args ) { setTimeout.apply(null,arguments) }
    /*#app*/ this.SetInterval = function( callback, delay, args ) { setInterval.apply(null,arguments) }
...
function Web( id )
{
    ...
    /*#web*/ obj.Reload = function( options ) { if(options&&options.toLowerCase().includes("ignoreerrors")) global.__ignoreErrors=true; prompt(obj.id,"Web.Reload(" ); global.__ignoreErrors=false; }
    ...
    /*#web*/ obj.SetOnBlob = function( callback ) { prompt( obj.id, "Web.SetOnBlob(\f"+_Cbm(callback) ); }
...
function Ply( id )
{
    ...
    /*#ply*/ obj.SetOnReady = function( callback ) { prompt( obj.id, "Ply.SetOnReady(\f"+_Cbm(callback) ); }
...
function Pst( id )
{
    ...
    /*#pst*/ obj.GetNumber = function() { return prompt( obj.id, "Pst.GetNumber(" ); }
    return obj;
}
...
function Cam( id )
{
    ...
    /*#cam*/ obj.Record = function( file,seconds,quality ) { prompt( obj.id, "Cam.Record(\f"+file+"\f"+seconds+"\f"+quality ); }
...
function Spr( id )
{
    ...
    /*#spr*/ obj.SetLanguage = function( langCode ) { prompt( obj.id, "Spr.SetLanguage(\f"+langCode ); }
...
if( _isPython ) _Cbm = function( func ) { return ( func ? new _ObjCbm(func).name : null ) }
else _Cbm = function( func ) { return ( func ? (window[func.name] ? func.name : (func._nohash?new _ObjCbm(func):new _ObjCbmH(func)).name) : null ); }
...
_LoadScriptSync = function( url, defer, type, id )
{
    ...
    if( _isV8 ) _ExecV8(url);
        ...
        if( url.endsWith(".py") ) type = "text/python"
        script.type = type ? type : 'text/javascript';
        if( id ) script.id = id
...
ide.Obfuscate = function( src,dest,options ) {
    if( app.InIDE() ) app.Broadcast( "!obfuscate", app.RealPath(src)+"|"+app.RealPath(dest)+"|"+options, "ide" )
}

ide.Export = function( type,src,dest ) {
    if( app.InIDE() ) app.Broadcast( "!export", type+"|"+app.RealPath(src)+"|"+app.RealPath(dest), "ide" )
}
...
ide.AddModule = function( name, overwrite )
{
    ...
    if( overwrite || !app.FolderExists(app.GetAppPath()+"/node_modules/"+name.split("@")[0]) ) {
        app.Broadcast( "!addmodule", name, "ide" )
        OnStart = function(){}
...
cfg = { Game:0, Landscape:0, Portrait:0, Transparent:0, Share:0, USB:0, Light:0, Dark:0,
        Holo:0, MUI:0, Node:0, NodeESM:0, Fast:0, Serv:0, ESM:0, Console:0, TV:0 }
